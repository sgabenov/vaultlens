...
| Policy privilege signals and escalation relationships | Cross-policy, role, entity and group fixtures | HCL policy signals feed auth detectors even when policy findings are disabled; POL-010..015 combine role assignments, token-role issuance and observed AppRole/Identity associations; nested group policy propagation and complete Identity coverage diagnostics pending |
...
AppRole association collection reads RoleID endpoints and the entity-alias catalog, persisting only SHA-256 RoleID/alias-name hashes plus canonical entity IDs and auth mount accessors. Existing snapshots without these fields can only support inferred authentication steps. Full alias display/export and nested group inheritance remain pending.
