...
import type { AuditDetail, AuditFinding } from '../../shared/securityAudit';
...
function Evidence({finding}:{finding:AuditFinding}) {
...
function FindingRows({findings,size}:{findings:AuditFinding[];size:number}) {
  ...
  return <div className="px-4">
    {findings.slice((page-1)*size,page*size).map((finding,index)=><details key={`${page}:${index}`} className="border-t py-3">
      <summary className="cursor-pointer text-sm"><span className="mr-2 text-xs font-medium">{finding.severity}</span><span className="break-all font-mono text-xs">{finding.namespace||'root'} · {finding.path}</span><span className="ml-2 text-xs text-gray-500">{finding.ruleId}</span></summary>
...
export default function AuditFindings({detail}:{detail:AuditDetail}) {
  ...
  const matched=useMemo(()=>detail.findings.filter(f=>(severity==='all'||f.severity===severity)&&(namespace==='all'||(f.namespace??'')===namespace)&&(category==='all'||ruleCategories.get(f.ruleId)===category)&&[f.path,f.title,f.ruleId,f.namespace??''].some(text=>text.toLowerCase().includes(query.toLowerCase()))),[detail.findings,severity,namespace,category,query,ruleCategories]);
  ...
  const filters=[{label:'Severity',value:severity,change:setSeverity,options:SEVERITIES.map(x=>[x,x])},{label:'Namespace',value:namespace,change:setNamespace,options:namespaces.map(x=>[x,x||'root'])},{label:'Category',value:category,change:setCategory,options:[...CHECK_GROUPS,'Legacy custom'].map(x=>[x,x])}];
  return <section className="space-y-4" aria-label="Audit findings">
    ...
    <div className="divide-y rounded border">{groups.slice((page-1)*size,page*size).map(group=><div key={group.key}>
      ...
      {expanded===group.key&&<FindingRows key={`${group.key}:${query}:${severity}:${namespace}:${category}:${size}`} findings={group.findings} size={size}/>}
    </div>)}</div>
