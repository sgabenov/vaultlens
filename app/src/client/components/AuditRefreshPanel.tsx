...
export default function AuditRefreshPanel({runId,disabled,controls,onRefreshed}:{runId:string;disabled:boolean;controls:{baselineYaml:string;exceptionsYaml:string};onRefreshed:(id:string)=>void}) {
  ...
  return <details className="rounded border p-3 text-sm">
    ...
    <p className="my-2 text-xs text-gray-500">Creates a new result using the saved collection scope, current rules and selected controls. Unrefreshed data remains from the previous snapshot. Auth role discovery reads auth mounts; refreshing mounts also reads auth roles.</p>
