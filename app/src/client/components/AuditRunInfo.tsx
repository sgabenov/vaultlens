...
function Fields({values}:{values:[string,unknown][]}) {
  return <dl className="grid gap-3 text-sm sm:grid-cols-2">{values.map(([label,value])=><div key={label}><dt className="text-gray-500">{label}</dt><dd className="break-words">{display(value)}</dd></div>)}</dl>;
}
export default function AuditRunInfo({runId,onClose}:{runId:string;onClose:()=>void}) {
  ...
  const policy=collection?.requestPolicy as Record<string,unknown>|undefined;
  const issues=[...(detail?.snapshot?.issues??[]),...(detail?.configuration?.issues??[])];
  const duration=run?.finishedAt?(new Date(run.finishedAt).getTime()-new Date(run.startedAt).getTime())/1000:undefined;
  return <dialog ref={ref} aria-labelledby="audit-run-info-title" onCancel={event=>{event.preventDefault();onClose();}}
    ...
    <div className="flex items-center justify-between gap-3"><h2 id="audit-run-info-title" className="text-lg font-semibold">Run info</h2><button autoFocus onClick={onClose} className="rounded border px-3 py-2 text-sm">Close</button></div>
    <p className="my-3 break-all font-mono text-xs text-gray-500">{runId}</p>
    {query.isPending&&<p>Loading run details…</p>}
    {query.error&&<p role="alert" className="text-sm text-red-700">Could not load this run.</p>}
    {run&&<div className="space-y-6">
      <Fields values={[
        ['Vault',run.target],['Status',run.status],['Started',new Date(run.startedAt).toLocaleString()],
      ...
      {detail?.snapshot?.refresh&&<div><h3 className="mb-3 font-medium">Selective refresh</h3><Fields values={[
        ['Refreshed sources',detail.snapshot.refresh.sources],['Retained resources',detail.snapshot.refresh.retainedResources],['Retained data from',detail.snapshot.refresh.retainedFrom],
      ]}/></div>}
      {collection?<>
        <div><h3 className="mb-3 font-medium">Collection scope and limits</h3><Fields values={[
          ['Namespace',policy?.namespace===''?'root':policy?.namespace],['Include child namespaces',policy?.recursiveNamespaces],
          ['Namespace filters',policy?.namespaceFilters],['Policy filters',collection.scope?.policyFilters],['Auth mount filters',collection.scope?.authMountFilters],['Auth type filters',collection.scope?.authTypeFilters],
          ['Skip Identity',collection.scope?.skipIdentity],['Workers',collection.workers],['Requests per second',policy?.requestsPerSecond],['Retries per request',policy?.retries],
          ['Request timeout (ms)',policy?.timeoutMs],['Duration limit (ms)',policy?.maxDurationMs],['Object limit',policy?.maxObjects===0?'Unlimited':policy?.maxObjects],['Omit full policy source',policy?.redactPolicySource],
        ]}/></div>
        <div><h3 className="mb-3 font-medium">Collection metrics</h3><Fields values={[
          ['Requests',collection.metrics.requests],['Retries',collection.metrics.retries],['Rate-limit wait (ms)',Math.round(collection.metrics.rateWaitMs)],['Retry wait (ms)',Math.round(collection.metrics.retryWaitMs)],
        ]}/></div>
