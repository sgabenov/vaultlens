      ...
      {!inGroup.length?<p className="rounded border p-4 text-sm text-gray-500">No dedicated built-in checks in this category yet. Collection of these objects does not imply they have passed a check.</p>:<>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(220px,1fr)_minmax(280px,1.4fr)]">
          ...
          {chosen&&<div className="space-y-4 rounded border p-5">
            ...
            <div className="text-sm"><label htmlFor="check-severity">Severity</label><div className="mt-2 flex flex-wrap items-center gap-3"><select id="check-severity" aria-label="Check severity" aria-describedby="check-default-severity" className="rounded border p-2" disabled={saving} value={severity(chosen)} onChange={event=>update([{path:['rules',chosen.id,'severity'],value:event.target.value}])}>{SEVERITIES.map(level=><option key={level} value={level}>{level}</option>)}</select><span id="check-default-severity" className="text-xs text-gray-500">Default: <span className="font-medium">{chosen.severity}</span></span></div></div>
