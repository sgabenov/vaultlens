    ...
    <section className="space-y-4">
      ...
      <div className="rounded border border-gray-200 p-4">
        ...
        <p className="my-2 text-sm text-gray-500">
          Live collection uses the Vault connection and authentication
          configured in VaultLens.
        </p>
        <Link
      ...
      <section className="space-y-3">
        ...
        {runId ? (
          <>
            <p className="text-sm text-gray-500">
              ...
              <Link
                ...
                to={`/security-audit/runs?${new URLSearchParams({ run: runId })}`}
              >
                Choose another run
              </Link>
            </p>
            ...
            {detail.error && (
              <p role="alert" className="text-sm text-red-700">
                Could not load the selected snapshot.
              </p>
            )}
            {detail.data &&
              (detail.data.snapshot?.collection &&
              ['collected', 'completed', 'partial'].includes(
                ...
                <AuditRefreshPanel
                  ...
                  runId={runId}
                  disabled={disabled}
                  controls={{ baselineYaml: '', exceptionsYaml: '' }}
                  onRefreshed={openResult}
                />
              ) : (
                <p className="text-sm text-gray-500">
                  This run has no completed collection to refresh. Select
                  another run or start a new collection.
                </p>
              ))}
          </>
        ) : (
          ...
          !runs.error && (
            <p className="text-sm text-gray-500">
              Collect a snapshot before refreshing selected sources.
            </p>
          )
        )}
      </section>
      {runs.error && (
        <p role="alert">
          Could not check running audits. Retry before refreshing or importing a
          snapshot.
        </p>
      )}
      <AuditImportUpload disabled={disabled} onImported={openResult} />
      <p className="text-sm text-gray-500">
        Imported snapshots are stored separately. Analyze them explicitly to
        produce findings.
      </p>
    </section>
  );
