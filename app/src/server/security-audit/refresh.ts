...
export function mergeRefresh(base:AuditSnapshot,fresh:AuditSnapshot,sources:string[]):AuditSnapshot {
  ...
  const policyCoverage:Record<string,boolean>={},aliasCoverage:Record<string,boolean>={};
  const refreshedStages=new Set(fresh.collection?.stageResults?.map(stage=>stageKey(stage.namespace,stage.stage))??[]);
  for(const namespace of namespaces) {
    policyCoverage[namespace]=refreshedStages.has(stageKey(namespace,'Policies'))?fresh.namespacePolicyCompleteness?.[namespace]??false:base.namespacePolicyCompleteness?.[namespace]??base.policiesComplete;
    aliasCoverage[namespace]=refreshedStages.has(stageKey(namespace,'Identity aliases'))?fresh.namespaceAliasCompleteness?.[namespace]??false:base.namespaceAliasCompleteness?.[namespace]??false;
  }
  return {...fresh,resources:[...resources.values()],namespaces,namespacePolicyCompleteness:policyCoverage,namespaceAliasCompleteness:aliasCoverage,
