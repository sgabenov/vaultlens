...
export function parseExceptions(source:string, ruleIds:Set<string>): FindingException[] {
  if(Buffer.byteLength(source)>256*1024) throw new Error('Exceptions exceed 256 KiB');
  ...
  if(!raw || typeof raw!=='object' || Array.isArray(raw) || raw.version!==1 ||
    Object.keys(raw).some(k=>!['version','exceptions'].includes(k))) throw new Error('Expected version 1 exceptions');
  const entries=raw.exceptions??[];
  if(!Array.isArray(entries) || entries.length>1000) throw new Error('Expected at most 1000 exceptions');
  ...
  return entries.map((entry,index)=>{
    ...
    if(seen.has(entry.id)) throw new Error(`Duplicate exception id: ${entry.id}`);
    seen.add(entry.id);
    if(!ruleIds.has(entry.rule_id)) throw new Error(`Unknown exception rule: ${entry.rule_id}`);
    if(!validDate(entry.expires)) throw new Error('Exception expires must be YYYY-MM-DD');
    if(entry.policy_path!==undefined && entry.policy_path!==null) {
      if(typeof entry.policy_path!=='string'||!entry.policy_path.trim()) throw new Error('Invalid exception policy_path');
