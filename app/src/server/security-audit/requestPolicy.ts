...
export interface CollectionOptions extends RequestPolicyOptions {
  namespace:string;redactPolicySource:boolean;
...
export function parseCollectionOptions(raw:unknown):CollectionOptions {
  ...
  if(Object.keys(value).some(key=>!['workers','retries','requestsPerSecond','retryBackoffMs','timeoutMs','maxDurationMs','maxObjects','policyFilters','authMountFilters','authTypeFilters','skipIdentity','namespace','redactPolicySource'].includes(key))) throw new Error('Unknown collection option');
  const options={...DEFAULT_REQUEST_POLICY,workers:10,timeoutMs:30000,maxDurationMs:7200000,maxObjects:0,policyFilters:[],authMountFilters:[],authTypeFilters:[],skipIdentity:false,redactPolicySource:false,namespace:'',...value} as CollectionOptions;
