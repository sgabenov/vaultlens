...
export function findingFingerprint(finding: AuditFinding): string {
  ...
  return createHash('sha256').update(canonical({namespace:finding.namespace??'',rule:finding.ruleId,path:finding.path,
...
export function createBaseline(detail: AuditDetail): AuditBaseline {
  ...
  return {version:1,tool:'vaultlens',namespaces:snapshotNamespaces(detail.snapshot),target:detail.snapshot.target,source_scan_id:detail.run.id,
...
export function parseBaseline(source:string): AuditBaseline {
  ...
  const allowed=['version','tool','target','source_scan_id','config_hash','engine_version','fingerprints','namespaces'];
  ...
  if(raw.namespaces!==undefined && (!Array.isArray(raw.namespaces)||raw.namespaces.some((v:unknown)=>typeof v!=='string'))) throw new Error('Invalid baseline namespaces');
  return raw as AuditBaseline;
}
export function applyBaseline(findings:AuditFinding[], configuration:RunConfiguration, target:string, baseline?:AuditBaseline, exceptions:FindingException[]=[], today=localDate(),namespaces:string[]=['']):import('../../shared/securityAudit.js').AuditControls {
  ...
  if(baseline && canonical([...(baseline.namespaces??[''])].sort())!==canonical([...new Set(namespaces)].sort())) throw new Error('Baseline namespaces are incompatible');
