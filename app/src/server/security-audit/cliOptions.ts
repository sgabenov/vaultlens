...
export function parseAuditArguments(args:string[]) {
  ...
  const filters:Record<string,string[]>={'--source':[],'--namespace-filter':[],'--policy-filter':[],'--auth-mount-filter':[],'--auth-type-filter':[]};
  const arities:Record<string,[number,number]>={'export-archive':[2,2],'export-directory':[2,2],refresh:[1,1],resume:[1,1],'import-python':[1,1],export:[2,2],collect:[0,0],scan:[0,0],analyze:[1,1],diff:[2,2],'baseline-create':[2,2],rules:[0,0],list:[0,0],configure:[1,2]};
  if(!command || !arities[command]) throw new Error(`Expected one of: ${Object.keys(arities).join(', ')}`);
  const analysis=['refresh','resume','scan','analyze'].includes(command), gating=analysis||command==='diff';
  for(let i=1;i<args.length;i++) {
    const token=args[i];
    if(!token.startsWith('--')) {positionals.push(token);continue;}
    if(Object.hasOwn(filters,token)) {
      if(!['scan','collect'].includes(command) && !(command==='refresh' && token==='--source')) throw new Error(`Unsupported option ${token} for ${command}`);
    ...
    if(token==='--checkpoint-max-age-ms' && command==='resume' || ['--baseline','--exceptions'].includes(token) && analysis || token==='--fail-on' && gating || token==='--format' && command==='export' || ['--namespace','--policy-filter','--auth-mount-filter','--auth-type-filter','--max-objects','--timeout-ms','--max-duration-ms','--workers','--retries','--requests-per-second','--retry-backoff-ms'].includes(token) && ['scan','collect'].includes(command)) {
      const value=args[++i]; if(!value || value.startsWith('--')) throw new Error(`${token} requires a value`);
      options[token]=value;
    } else if(token==='--recursive-namespaces' && ['scan','collect'].includes(command)) options[token]=true;
    else if(token==='--redact-policy-source' && ['export-archive','export-directory','export','scan','collect'].includes(command)) options[token]=true;
  ...
  const checkpointMaxAgeMs=Number(options['--checkpoint-max-age-ms']??86400000);
  if(!Number.isSafeInteger(checkpointMaxAgeMs)||checkpointMaxAgeMs<1||checkpointMaxAgeMs>604800000) throw new Error('Invalid checkpoint max age');
