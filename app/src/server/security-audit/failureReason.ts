...
export function importFailureReason(error:unknown):string {
  ...
  if (message==='Only Python snapshot schema 3 is supported')
    return 'Unsupported Python snapshot schema. Export a schema-3 snapshot.';
  if (message==='Import requires a finished Python snapshot with valid timestamps')
  ...
  if (message==='Python snapshot exceeds 256 MiB' || message==='Python snapshot exceeds one million records')
