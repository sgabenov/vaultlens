...
const { dbPath, id, target, token, skipTlsVerify, settings, collectionOptions } = workerData;
const store = new AuditStore(dbPath);
try {
  const snapshot = await collect(target, token, skipTlsVerify, collectionOptions);
