...
const { resumeRunId, checkpointMaxAgeMs, importPath, dbPath, id, target, token, skipTlsVerify, settings, collectionOptions, sourceRunId, baseline, exceptions } = workerData;
...
try {
  ...
  const snapshot = sourceRunId ? store.get(sourceRunId,target)?.snapshot : await collect(target, token, skipTlsVerify, collectionOptions,progress,snapshot=>store.saveCheckpoint(id,snapshot),resume);
  if(!snapshot) throw new Error('Source snapshot not found');
  if(sourceRunId || resumeRunId) snapshot.sourceRunId=sourceRunId??resumeRunId;
