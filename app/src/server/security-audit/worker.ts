...
const { refreshRunId, resumeRunId, checkpointMaxAgeMs, importPath, dbPath, id, target, token, skipTlsVerify, settings, collectionOptions, sourceRunId, baseline, exceptions } = workerData;
...
try {
  ...
  if(resumeRunId && (!resumeSource?.snapshot || !['failed','interrupted'].includes(resumeSource.run.status))) throw new Error('Checkpoint unavailable');
  const resume=resumeSource?.snapshot?{snapshot:resumeSource.snapshot,maxAgeMs:checkpointMaxAgeMs}:undefined;
  const refreshSource=refreshRunId?store.get(refreshRunId,target):null;
  if(refreshRunId && (!refreshSource?.snapshot || !['collected','completed','partial'].includes(refreshSource.run.status))) throw new Error('Refresh source unavailable');
  let snapshot = sourceRunId ? store.get(sourceRunId,target)?.snapshot : await collect(target, token, skipTlsVerify, collectionOptions,progress,snapshot=>store.saveCheckpoint(id,snapshot),resume);
  if(!snapshot) throw new Error('Source snapshot not found');
  if(refreshSource?.snapshot) snapshot=mergeRefresh(refreshSource.snapshot,snapshot,collectionOptions.sources);
  if(sourceRunId || resumeRunId || refreshRunId) snapshot.sourceRunId=sourceRunId??resumeRunId??refreshRunId;
